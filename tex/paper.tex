...
21$^\dagger$ & Buildings \& structures & Per-kW scaling, fuel-dependent (19 building types) \\
...
\hspace{1em}22.01.03 & Coils (magnets) & Conductor kAm $\times$ \$/kAm $\times$ markup \\
...
The cost model sums per-kW costs (\$/kW of gross electric) across
19~building types, with fuel-dependent scaling on four tritium-related
buildings.

The total building cost is ${\sim}$760~\$/kW for D-T
(${\sim}$511~\$/kW for non-D-T), yielding ${\sim}$\$886M at 1~GWe net.
This represents ${\sim}$14\% of overnight capital cost---consistent with
the NREL ATB 2024 nuclear cost structure (16.1\% for structures), after
discounting for non-nuclear-grade construction under Part~30.

\subsubsection{Fuel-dependent scaling.}
Four buildings associated with tritium handling receive a 0.5$\times$
scaling factor for non-D-T fuels: site improvements (tritium monitoring
perimeter), fusion heat island (tritium barrier systems), hot cell
(activated component handling), and fuel storage.  These four buildings
account for 65\% of D-T building cost; the 0.5$\times$ factor yields a
33\% reduction for aneutronic fuels.  The scaling is binary (D-T vs.\
non-D-T) rather than fuel-specific, as building cost uncertainty
(${\sim}\pm$30\%) dominates the fuel-scaling uncertainty.

Source: ARIES cost-account tradition \citep{waganer2013}, calibrated
against the NETL baseline \citep{netl2022}.
