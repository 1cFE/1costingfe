"""PROTOTYPE: dec_blog_numbers.py with PULSED_FRC for the Helion case.

The original keeps the D-He3 "pulsed inductive" rows on a MIRROR concept
and reconstructs pulsed-inductive economics by hand (PI_BURN dict +
cost overrides for CAS21/22/23/24/26). This copy swaps those rows to
ConfinementConcept.PULSED_FRC so the framework's native pulsed model
sets CAS22 / CAS23 / etc., with no hand-rolled cost overrides.
"""
...
m_dhe3_frc = CostModel(
    concept=ConfinementConcept.PULSED_FRC,
    fuel=Fuel.DHE3,
    power_cycle=PowerCycle.BRAYTON_SCO2,  # ignored for INDUCTIVE_DEC
)
...
# Pulsed inductive D-He3 via PULSED_FRC concept (Helion-likely).
# The framework's pulsed model sets CAS22 (cap bank + DC-DC + inverter),
# zeros CAS23 (no turbine — INDUCTIVE_DEC), and trims BOP accounts natively.
# We still override the burn physics to the D-rich operating point:
#   dhe3_dd_frac=0.314, f_rad=0.163 (vs. PULSED_FRC YAML defaults 0.131, 0.05)
# eta_dec / eta_pin / f_pdv come from the PULSED_FRC YAML.
PI_BURN_FRC = dict(
    dhe3_dd_frac=0.314,
    f_rad=0.163,
)
r_dhe3_pi = m_dhe3_frc.forward(**BASE_KW, inflation_rate=INFLATION, **PI_BURN_FRC)
...
# Pulsed inductive D-He3 (free-core BOP floor via PULSED_FRC):
# Free the core (CAS22 reactor plant + CAS27 special materials) so we
# isolate the pulsed-inductive BOP floor. The framework still computes
# CAS21/CAS23/CAS24/CAS26 natively for the pulsed concept.
r_p_dhe3 = m_dhe3_frc.forward(
    **BASE_KW,
    inflation_rate=INFLATION,
    **PI_BURN_FRC,
    cost_overrides=FREE_CORE_DHE3,
)
