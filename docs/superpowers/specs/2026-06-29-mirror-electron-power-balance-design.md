...
- Electron thermal-conduction share (gamma_e > 0) / explicit mirror electron
  end-loss; we use the paper's gamma_e = 0.
...
Steady-state electron balance (eta_alpha = 0, thermal), solved for `T_e`:
    ...
    alpha_e0(T_e) * P_alpha,ret  +  K_ie(T_e) * (T_i - T_e)   =   P_B(T_e)
    [ alpha -> electron heating ]   [ ion-electron equilibration ]  [ bremsstrahlung ]

with the electron thermal-conduction share set to zero (paper Eq. 15: electrons
lose energy only through bremsstrahlung). Synchrotron / ECR stays OFF this
right-hand side; it is device-specific and largely reabsorbed (paper Sec. VIII.B),
and is handled separately in the existing radiation accounting.
...
5. `gamma_e = 0`: electrons carry none of the thermal-conduction loss.
