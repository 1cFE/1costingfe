...
CAS22        & Reactor plant equipment                      & 1889.1 \\
CAS23--29    & Other direct (BOP, special materials, contingency) & 235.9 \\
CAS30        & Capitalized indirect services                & 489.4 \\
CAS40        & Owner's costs                                & 23.0 \\
CAS50        & Supplementary costs                          & 183.8 \\
CAS60        & Interest during construction                 & 607.0 \\
\midrule
\multicolumn{2}{l}{Total overnight}                          & 3764.7 \\
\multicolumn{2}{l}{Overnight specific cost (\$/kW)}          & 3765 \\
\midrule
CAS70 (M\$/yr) & O\&M and scheduled replacement             & 34.5 \\
CAS80 (M\$/yr) & Fuel (raw isotopes)                        & 256.4 \\
CAS90 (M\$/yr) & Annualized financial (CRF, \cref{eq:crf})   & 303.4 \\
\midrule
\multicolumn{2}{l}{\textbf{LCOE (\$/MWh)}}                  & \textbf{78.0} \\
\bottomrule
\end{tabular}
\end{table}
...
parameter is \texttt{fuel\_recovery} (elasticity $-7.33$), in the
physics-outputs band; the most consequential cost unit price is
\texttt{u\_he3}, the ${}^3$He purchase price ($+0.44$); and the most
...
($T_e \sim 10$--$30$~keV), and it is applied (summed with synchrotron and
...
As in the steady-state balance, we trace the per-pulse energy from the
...
Each building is costed for what it physically is; the per-fuel totals
(each a sum of the individually-costed buildings) are summarized in
\cref{tab:cas21-fuel}.

\begin{table}[htbp]
\caption{CAS21 buildings cost by fuel cycle.}
\label{tab:cas21-fuel}
\centering
\begin{tabular}{lrl}
\toprule
Fuel & Value at 1\,GWe (M\$) & Construction grade \\
\midrule
D-T    & 629 & Enhanced industrial (tritium confinement, hot cell, rad-HVAC) \\
D-D    & 576 & Enhanced industrial (reduced tritium/activation scope) \\
D-${}^3$He & 452 & Light enhanced industrial \\
p-${}^{11}$B & 385 & Industrial (no hot cell, no tritium, standard HVAC) \\
\bottomrule
\end{tabular}
\end{table}

\Cref{tab:cas21-buildings} lists each building with its per-fuel cost at the
1\,GWe reference and the power quantity it scales with.
...
The fuel dependence is an activation and tritium-handling story: the
buildings that shield neutrons or confine tritium (reactor building, hot
cell, ventilation/HVAC) ease from D-T through D-${}^3$He as the neutron
energy and tritium inventory fall, and the hot cell vanishes entirely for
aneutronic p-${}^{11}$B (nothing activates). Ventilation scales with
...
%% ------------------------------------------------------------
\subsubsection{CAS22.01.07: Power Supplies}
\label{sec:cas2207}
...
%% ------------------------------------------------------------
\subsubsection{CAS22.01.08: Divertor and Target Factory}
\label{sec:cas2208}
...
\subsubsection{CAS22.01.09: Direct Energy Converter (electrostatic)}
\label{sec:cas2209}
...
\item \textbf{CAS72}: Annualized scheduled component replacement.
  ...
  hardness) and $q_{\text{wall}}$ the neutron wall loading (neutron
  power per unit first-wall area). A higher-$q_{\text{wall}}$ machine wears its core out
...
Dense plasma focus & 0.01 & 0.005--0.02 & Sub-microsecond pinch lifetime is binding \citep{lerner2023dpf}; genuinely low and materially raises DPF LCOE. \\
\bottomrule
\end{tabular}
\end{table}
...
MW\,yr/m$^2$. Independent cross-checks bracket this window: the ARIES-AT
...
but the plasma-to-wall mapping is the model's.
