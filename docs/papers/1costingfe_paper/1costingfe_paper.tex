...
with the greatest leverage on the levelized cost of electricity (LCOE). 
In addition to the physics and
engineering parameters that determine the plant's performance, the
economic parameters (construction time, economic plant lifetime, capital
cost and operating costs, and the cost of capital, to name a few) become
...
regarding very early stage physics. As an example, direct energy conversion (DEC) methods are included across the
TRL range, from the demonstrated venetian-blind grid through pulsed-inductive
DEC and MHD on fusion coolants; their cost contributions are computed but
flagged as TRL-conditional.
...
\paragraph{Batch sweeps.} Vectorised via \texttt{vmap},
