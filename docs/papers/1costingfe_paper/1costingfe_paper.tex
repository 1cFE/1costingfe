...
1costingfe physics module. Costing parameters (WACC, $T_c$, NOAK
switch, conductor cost per kA-m, ${}^3$He market price, etc.) enter
at the engineering-sizing and CAS-account stages.
...
(\cref{sec:cas80}). The NOAK defaults are $f_b = 0.05$ and $f_r = 0.99$,
...
$f_{\text{DD}}$ of fusion events are D-D side reactions, which
themselves carry the D-T and D-${}^3$He secondaries described in the
D-D section above. The D-D events produce tritium (via D(d,p)T) that
re-burns in D-T fusion at burn fraction $f_T^*$, introducing 14.1~MeV
neutrons into an otherwise neutron-free system. They also produce
...
energy or exhaust to keep the design neutron-averse), kept separate
from the D-D fuel cycle's burn fractions and concept-specific. Mirror
plants without active tritium discharge let bred T burn at
$f_T^* \approx 0.5$ (single-pass thermal burn); pulsed FRC
architectures designed to be neutron-averse exhaust (or hold for
...
NOAK defaults $f_b = 0.05$ and $f_r = 0.99$, $f_{{}^3\text{He}}^*
\approx 0.84$.
...
sections \citep{boschhale1992}), $f_T^* = 0.5$, and the NOAK-default
...
stellarators, mirrors). It converts the fusion-power split of
