...
or accelerate a target every shot (laser and heavy-ion IFE, MagLIF, and
the pneumatic mag-target loop). Pulsed concepts that form their plasma
in situ from gas injection (pulsed FRC, theta pinch, dense plasma
focus, sheared-flow Z-pinch) manufacture no targets and carry
$C_{220108} = 0$.
...
separately under CAS27 (Section~\ref{sec:cas27}).
...
Table~\ref{tab:cas27_fills} lists the chemistry factors and their cost
...
The \$5\,M figure is anchored to disclosed advanced-reactor digital-twin
...
The Section~\ref{sec:tour} walkthrough is recorded by
...
The full reference script for the Section~\ref{sec:tour} walkthrough is
