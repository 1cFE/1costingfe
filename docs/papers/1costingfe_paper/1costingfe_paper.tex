...
quoted to two significant figures because the anchors are procurement
bands. The NBI cost is the midpoint of the ITER NBI procurement band
(EUR~9--15M/MW FOAK) after a 30--50\% NOAK learning discount; the ECRH
cost is mid-band ITER gyrotron procurement (EUR~5--10M/MW). ICRF and
...
exists, so both are NOAK estimates rather than procurement anchors.
They carry the same value because they are the same RF-source hardware
class, klystrons and transmitters with no beamline and no
superconducting content; the nearest published analog is a
superconducting e-beam accelerator projected at about \$1.6/W of beam
power.
