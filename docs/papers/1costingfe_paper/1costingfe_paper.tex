...
\end{equation}
with two discrete hardware classes declared by the design (the
...
2~MW/m$^2$ steady state; ITER first-wall-panel procurement anchor) and a
divertor-grade monoblock wall ($2.5$~M\$/m$^2$ NOAK, qualified to
10~MW/m$^2$; ITER divertor procurement anchor). The declared class is
audited against the machine's surface-flux cap. Discrete classes mirror the
hardware reality: qualified wall products exist as panels and as
high-heat-flux components, with nothing in between, and within a class the
cost is area-driven.
