...
The cryogenics building is the magnet cryoplant: it is present only for
superconducting-magnet concepts and is zero for normal-conducting
(copper) and magnet-free concepts, which carry no cryoplant.
...
$c_{\text{div}} = 60$~M\$, but its value is concept-dependent. Concepts
...
direct charged particles to the direct energy converter. For inertial and
...
The mirror carries two distinct bores. The central-cell solenoids enclose the
full radial build, so their bore is the vessel outer radius plus an assembly
standoff, $r_{\text{coil}} = r_{\text{vessel,out}} + 0.10$~m, and they sit at the
coil design field $b_{\text{center}}$. The end-plug coils sit at the throat,
where the plasma necks down by flux conservation and no breeding blanket
extends, so their bore is set by the throat plasma radius plus a throat standoff,
$r_{\text{coil}} = a/\sqrt{R_m} + 0.30$~m, and they carry the throat field
$R_m B_{\min}$. Small-bore high-field plug coils and large-bore lower-field
central solenoids are the cost-relevant distinction unique to the mirror's
two-class magnet set.
...
Mirror      &  1.7266$\times$ & 1.0 & \$513M & Two-class: $n_{\text{central}}=L/d_{\text{coil}}$ central solenoids at $b_{\text{center}}$ with bore $r_{\text{coil}} = r_{\text{vessel,out}} + 0.10$~m plus $n_{\text{plug}}=4$ plug coils at the throat field $R_m B_{\min}$ with bore $r_{\text{coil}} = a/\sqrt{R_m} + 0.30$~m; calibrated at 20~m default \\
...
\item \textbf{CAS72}: Annualized scheduled component replacement.
  ...
  each replacement event and annualized via the CRF. For the
  steady-state magnetic-confinement concepts (tokamak, stellarator,
  mirror, steady-state FRC) the core first-wall and blanket are replaced
  on a neutron-fluence schedule: the full-power-year lifetime is the
  per-fuel fluence limit divided by the neutron wall loading,
  \begin{equation}
  \label{eq:core-lifetime}
  L_{\text{core}} = \mathrm{clamp}\!\left(
    \frac{\Phi_{\max}(f)}{q_{\text{wall}}},\;
    L_{\text{floor}},\; L_{\text{plant}}\right),
  \end{equation}
  ...
  hardness) and $q_{\text{wall}}$ the neutron wall loading of
  \cref{eq:q-wall}. A higher-$q_{\text{wall}}$ machine wears its core out
  sooner and carries a larger annualized replacement charge; the lifetime
  is clamped between a 0.5~FPY floor (keeping the $1/q_{\text{wall}}$
  gradient finite at extreme wall loading) and the plant full-power life
  (nothing is replaced beyond it). Limits and sources are documented in
  \texttt{docs/account\_justification/wall\_limits\_and\_fluence.md}. For
  pulsed
...
\label{sec:tokamak-anchors}

The tokamak closed energy balance is the template the mirror sustainment
model of \cref{sec:mirror-balance} copies, so the reference is itself
pinned to published design points, parallel to the mirror GDT/WHAM anchors
of \cref{sec:mirror-anchors}. Run at each machine's published
volume-averaged density and temperature with the published auxiliary power,
the unchanged forward model reproduces the published fusion gain $Q$ within
a factor 1.09 for ITER \citep{shimada2007} and 0.83 for ARC
\citep{sorbom2015arc}, both inside the 2x gate, and within a factor of
about three for SPARC \citep{creely2020}, the larger gap reflecting SPARC's
strongly peaked profiles against the flat-profile 0D idealization. As with
the mirror anchors, the 2x band exists for the flat-versus-peaked profile
gap rather than a calibration freedom; no tokamak physics or cost
coefficient is adjusted to the anchors.
...
\subsection{Confining Potential: Tandem Plug}
\label{sec:mirror-phi}

The reactor-relevant mirror is a tandem: the central cell that does the
fusion is confined by an electrostatic potential drop to high-field end
plugs, not by the simple-mirror electron-loss potential. Following
\citet{fowler1977} the central-cell ion confining potential is set by the
plug-to-central density ratio $n_p/n_c$:
%
\begin{equation}
\label{eq:phi-plug}
e\phi = T_{e,\text{plug}}\,\ln\!\left(\frac{n_p}{n_c}\right),
\end{equation}
%
where $T_{e,\text{plug}}$ is the plug hot-electron temperature. The plug is
a separate cell whose hot-electron population, sustained by plug ECH, holds
the confining potential, so $T_{e,\text{plug}}$ is decoupled from the
central-cell electron temperature \citep{baldwin1979}: a thermal barrier
lets the plug run hot (default $T_{e,\text{plug}} = 125$~keV, anchored to
the high-field tandem design point of \citet{frank2024}) while the central
cell sets its own $T_e$ for radiation, beta, and stored energy. Because
$n_p/n_c$ is an order-unity-to-few ratio, $e\phi$ is bounded, and at the
anchor ($n_c/n_p = 0.55$, $T_{e,\text{plug}} = 125$~keV) it evaluates to
$e\phi = 74.7$~keV, matching the published tandem central-cell value.

The simple-mirror Boltzmann ambipolar potential
%
\begin{equation}
\label{eq:phi-ambipolar}
e\phi_{\text{amb}} = T_e \ln\!\sqrt{\frac{A\,m_p}{2\pi\,m_e}}
\end{equation}
%
(about $3.2\,T_e$ for deuterium) is retained as a diagnostic for the
simple-mirror limit; the tandem confining potential of
\cref{eq:phi-plug} is the one entering the confinement chain.
...
\subsubsection{Combined Axial Confinement: Collisionality Bridge}

The Pastukhov and gas-dynamic channels apply in physically distinct
collisionality regimes and must be joined by a bridge that selects the
governing limit. \citet{rognlien1980} establish that the Pastukhov
loss-cone formula holds in the collisionless limit, while the collisional
gas-dynamic outflow governs once the ion mean free path, reduced by the
mirror ratio, falls to the order of the device length; the regime boundary
is therefore at collisionality
%
\begin{equation}
\label{eq:collisionality}
\xi \equiv \frac{L}{v_{ti}\,\tau_{ii}} \sim \frac{1}{R_m},
\end{equation}
%
with $\xi \gg 1/R_m$ collisional and $\xi \ll 1/R_m$ collisionless. The
gas-dynamic loss rate physically exists only when the loss cone is kept
full by collisions, so it is gated by a smooth logistic in $\log_{10}\xi$
centered on the boundary and combined via the loss-rate sum:
%
\begin{align}
\label{eq:regime-gate}
g(\xi) &= \sigma\!\left(\frac{\log_{10}\xi - \log_{10}(1/R_m)}{w}\right), \\
\label{eq:tau-axial}
\frac{1}{\tau_{\text{axial}}} &= \frac{1}{\tau_{\text{Pas}}}
  + g(\xi)\,\frac{1}{\tau_{\text{GD}}},
\end{align}
%
where $\sigma$ is the logistic function and $w = 0.13$ decades is a
constructed smoothing width (\citeauthor{rognlien1980} establish the
transition is smooth but publish no closed-form width). As $g \to 1$ in
the collisional regime the gas-dynamic time dominates and
$\tau_{\text{axial}} \to \tau_{\text{GD}}$; as $g \to 0$ in the
collisionless regime the gas-dynamic rate is suppressed below the Pastukhov
rate and $\tau_{\text{axial}} \to \tau_{\text{Pas}}$. The logistic argument
is clamped to $[-30,30]$ before exponentiation to keep the reverse-mode
gradient finite. The total particle confinement combines axial and radial:
%
\begin{equation}
...
\subsection{Alpha Loss-Cone Heating}
\label{sec:mirror-alpha}

A mirror does not confine its fusion alphas the way a closed device does:
a fraction of the 3.5~MeV alphas is born in or scatters into the
velocity-space loss cone and streams out the ends before depositing its
energy. \citet{santarius1983} give a bounce-averaged Fokker-Planck result
that roughly half the alphas are lost by count but under a quarter by
energy, so $f_{\alpha,\text{heat}} \approx 0.75$--$0.85$ of the
charged-particle power deposits in the central cell (model default
$f_{\alpha,\text{heat}} = 0.80$). The model credits only
$f_{\alpha,\text{heat}}\,P_\alpha$ as self-heating and routes the
remaining $(1 - f_{\alpha,\text{heat}})\,P_\alpha$ to the axial end-loss
channel as directed loss-cone exhaust, where it is recovered by the
end-plug direct converter alongside the thermal end loss; the alpha energy
is conserved, not discarded.

\subsection{Energy-Balance Closure and Plug Power}
\label{sec:mirror-balance}

In sizing mode the auxiliary sustainment power is charged from the closed
steady-state energy balance rather than asserted. The confinement-derived
sustainment is
%
\begin{equation}
\label{eq:mirror-aux}
P_{\text{aux}} = \max\!\left(P_{\text{aux}}^{\text{floor}},\;
  P_{\text{end}} + P_{\text{radial}} + P_{\text{rad}}
  - f_{\alpha,\text{heat}}\,P_\alpha\right),
\end{equation}
%
which is fed as the heating input to the shared MFE power balance of
\cref{sec:inverse-balance}; the transport channel then conserves the
undeposited alpha power, $p_{\text{transport}} = P_{\text{end}} +
P_{\text{radial}} + (1 - f_{\alpha,\text{heat}})\,P_\alpha$, so the
fusion-to-injected gain $Q_{\text{sci}} = P_{\text{fus}}/P_{\text{aux}}$
and the engineering gain follow self-consistently from the confinement.
The hot-electron plug is sustained at a real power cost: a plug power
$P_{\text{plug}}$ (default 30~MW, anchored to the high-field tandem design
point of \citet{frank2024}) is charged into the recirculating budget,
since the plug ECH heats the plug electrons rather than the central cell.
This plug power is the competing penalty that keeps the optimizer honest:
deep plug confinement is not free.

Because the Fowler-Logan potential of \cref{eq:phi-plug} is fixed by the
plug rather than scaling with $T_i$, the ratio $e\phi/T_i$ falls as the
central cell is heated, so the Pastukhov enhancement weakens with $T_i$:
heating the central cell costs confinement. The length-sizing and
LCOE-optimize search therefore settles the D-T central cell at a cool,
genuinely driven operating point ($T_i \approx 23$~keV) with auxiliary
sustainment well above the control floor, rather than running to ignition.
...
For length sizing ($P_{\text{net}} \to L$), the operating density is the
minimum of three caps,
%
\begin{equation}
\label{eq:n-resolution}
n_e = \min\!\left(n_\beta,\; n_{\text{wall}},\; n_{\text{surf}}\right),
\end{equation}
%
each the density at which a distinct first-wall or stability limit binds.
The beta-pressure cap sets the density at a fraction $f_\beta$ of the MHD
stability boundary:
%
\begin{equation}
\label{eq:n-fbeta}
n_\beta = f_\beta\,\beta_{\max}\,
  \frac{B_{\min}^2}{2\,\mu_0\,(T_e + n_i/n_e\cdot T_i)\,e_{\text{keV}}}
\end{equation}
%
The neutron wall-load cap limits the 14.1~MeV fluence on the first wall.
Writing the neutron wall loading as
%
\begin{equation}
\label{eq:q-wall}
q_{\text{wall}} = \frac{P_{\text{neutron}}}{A_{\text{fw}}},
  \qquad A_{\text{fw}} = 2\pi\,(a + t_{\text{vac}})\,L,
\end{equation}
%
and using $P_{\text{neutron}} = f_n\,C_{\text{fus}}(T)\,n_e^2\,V$ with the
plasma volume $V = \pi a^2 L$ (the $L$ cancels, so the cap is set by $a$,
$n_e$, and $T$ alone), the wall-cap density follows in closed form,
%
\begin{equation}
\label{eq:n-wall}
n_{\text{wall}} = \sqrt{\frac{2\,q_{\text{wall}}^{\max}\,(a + t_{\text{vac}})}
  {f_n\,C_{\text{fus}}(T)\,a^2}},
\end{equation}
%
where $f_n$ is the neutron energy fraction and $C_{\text{fus}}(T)$ the
per-$n_e^2$ fusion power density coefficient. The surface heat-flux cap
limits the photon and cross-field charged-particle load deposited in the
first microns of the wall,
%
\begin{equation}
\label{eq:q-surface}
q_{\text{surface}} = \frac{P_{\text{rad}} + P_{\text{radial}}}{A_{\text{fw}}}
  \le q_{\text{surface}}^{\max},
\end{equation}
%
with the axial end-loss excluded (it exits through the throats to the
expander, not the lateral wall). Because radiation plus radial transport
is monotone increasing in density at fixed $T$, $n_{\text{surf}}$ is found
by a single uniform bisection on $n_e$ for all fuels. The default caps are
$q_{\text{wall}}^{\max} = 5$~MW/m$^2$ and $q_{\text{surface}}^{\max} =
1$~MW/m$^2$, sourced in the wall-limits provenance writeup. For D-T the
neutron cap typically binds first; for advanced fuels the surface cap is
the governing wall constraint. At the resolved density $P_{\text{fus}}
\propto n_e^2 V \propto n_e^2 L$ is monotonically increasing in $L$ (for
fixed $n_e$), so $L$ is located by bisection on $[L_{\min}, L_{\max}]$. If
$L_{\max}$ is insufficient, or if even the capped density cannot reach
$P_{\text{net}}^*$, a \texttt{SizingInfeasible} error is raised.
...
JAX-differentiable), at the design field $b_{\text{center}}$ and a bore
$r_{\text{coil}} = r_{\text{vessel,out}} + 0.10$~m taken from the radial
build, and (ii) end-plug coils with fixed count $n_{\text{plug}}$ (default
4, two per end), at the throat field $R_m B_{\min}$ and a bore
$r_{\text{coil}} = a/\sqrt{R_m} + 0.30$~m set by throat flux conservation.
The two bores differ in kind: the central solenoids enclose the full
plasma--blanket--shield--vessel stack, while the plug coils sit where the
plasma necks down and no blanket extends. The total coil cost scales as a
linear combination of the two counts, carrying the mirror markup
$M = 1.7266$ calibrated at the YAML default chamber length of 20~m
($n_{\text{central}} = 4$, $n_{\text{plug}} = 4$, baseline 513~M\$).
...
the published scaling.  Within the 2x gate. The collisionality bridge of
\cref{eq:tau-axial} places GDT on the gas-dynamic branch (collisional,
$\xi$ above $1/R_m$) and WHAM on the Pastukhov branch (collisionless, $\xi$
more than three decades below $1/R_m$), each within one percent of the
governing kernel.

\paragraph{Hammir (tandem anchor.)}
The high-field axisymmetric tandem design point of \citet{frank2024} has a
50~m central cell, $n_c/n_p = 0.55$, $T_i = 45$~keV,
$T_{e,\text{plug}} = 125$~keV, $P_{\text{fus}} = 157$~MW, and 30~MW of
plug NBI/ECH, giving a published gain $Q = P_{\text{fus}}/P_{\text{NBI}}
\approx 5.2$. At this central cell the tandem confining potential of
\cref{eq:phi-plug} gives $e\phi = 74.7$~keV, the central cell is not
ignited, and the model gain lands within the 2x band; charging
$P_{\text{plug}} = 30$~MW makes the engineering-gain accounting match the
published $Q$, which counts the same plug power.

\subsection{Headline Operating Point}
\label{sec:mirror-headline}

Sized to a 400~MWe net target at the default high-field tandem
($f_\beta = 0.85$), the D-T mirror is a driven tandem at $T_i = 23$~keV
with $Q_{\text{sci}} = 8.3$, energy confinement $\tau_E = 19$~s, auxiliary
sustainment near 245~MW, engineering gain $Q_{\text{eng}} = 1.68$, and
LCOE about 396~\$/MWh; the operating point is beta-bound. D-T is therefore
net-positive but marginal, paying the full recirculating cost of its
hot-electron plug. The advanced fuels (D-D, D-${}^3$He, p-${}^{11}$B) are
each genuinely driven but net-negative at the modelled fields: the
hot-electron plug needed to confine the central cell radiates against
their lower or harder reactivity, so none reaches $Q_{\text{eng}} > 1$
even with a cool central cell. Only D-T is economic in the 0D model.
...
\item The central-cell $T_e$ and the plug $T_{e,\text{plug}}$ are inputs;
  there is no self-consistent electron power balance within the plug.
...
\item The plug is modeled as a confining potential plus a fixed
  sustainment power, not a self-consistent second fusion plasma; the
  plug density ratio and plug power are inputs anchored to a single
  published design point.
\item MHD stability is enforced only via the $\beta \le \beta_{\max}$
  gate. A collisionality-validity flag and a DCLC loss-cone proxy are
  reported as informational diagnostics, but drift-resonance and
  mirror-mode instabilities do not restrict the operating point.
