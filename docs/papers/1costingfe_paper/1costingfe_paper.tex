...
on-site target factory, $C_{220108} = c_{\text{fac}}\,(P_{\text{net}}/1000)^{0.7}$,
where the capital $c_{\text{fac}}$ is a \emph{per-concept} bottom-up
build-up rather than a single global base. A laser or heavy-ion capsule
factory is a precision cleanroom (capsule surface and sphericity held to
a few tenths of a percent) whose tritium-handling back half---DT fill,
ice-layering, contained assembly and injection---adds a dual-confinement
premium on top, and whose throughput must be oversized by the inverse
acceptance yield; this lands at $c_{\text{fac}}\approx 725$--$780$~M\$ at
1~GWe, calibrated so its optimistic corner reproduces the
LIFE~\cite{anklam2011life} target factory ($\sim$\$600~M, \$0.25/target).
A MagLIF or Z-pinch load is instead formed by stamping and filling a
metal liner, a casting-shop process costing $\sim$$150$~M\$---roughly
five times cheaper, and not the capsule cleanroom that an earlier flat
$c_{\text{tgt}} = 244$~M\$ phantom-applied to every concept. That flat
number, inherited from the Miles/LIFE factory total via
pyFECONS~\\cite{woodruff2026}, also double-counted the factory because
the per-shot target cost (\cref{sec:cas80}) is sourced from fully-loaded
studies that already amortize the plant; the capital and per-shot
accounts are disentangled here so the factory is counted once.
