...
CAS21        & Buildings \& structures                      & 323.2 \\
CAS22        & Reactor plant equipment                      & 1730.7 \\
CAS23--29    & Other direct (BOP, special materials, contingency) & 235.9 \\
CAS30        & Capitalized indirect services                & 458.0 \\
CAS40        & Owner's costs                                & 23.0 \\
CAS50        & Supplementary costs                          & 177.0 \\
CAS60        & Interest during construction                 & 569.4 \\
\midrule
\multicolumn{2}{l}{Total overnight}                          & 3531.7 \\
\multicolumn{2}{l}{Overnight specific cost (\$/kW)}          & 3532 \\
\midrule
CAS70 (M\$/yr) & O\&M and scheduled replacement             & 34.5 \\
CAS80 (M\$/yr) & Fuel                                       & 256.4 \\
CAS90 (M\$/yr) & Annualized financial (CRF $\times$ capital) & 284.6 \\
\midrule
\multicolumn{2}{l}{\textbf{LCOE (\$/MWh)}}                  & \textbf{75.5} \\
\bottomrule
\end{tabular}
\end{table}
...
27$^\dagger$ & Special materials & Extended & Fuel-dependent base $\times$ blanket-fill chemistry factor (PbLi / Li / FLiBe / Be ceramic / ceramic only / none) \\
...
target factory, $C_{220108} = c_{\text{tgt}}\,(P_{\text{et}}/1000)^{0.7}$
with $c_{\text{tgt}} = 244$~M\$, applied to the concepts that fabricate
a consumed target or liner every shot (laser and heavy-ion IFE, MagLIF,
and the wire-array / dynamic-hohlraum Z-pinch). Whether a concept
carries this factory is set by the single per-shot target cost
$c_{\text{target}}$ (\cref{sec:cas80}): concepts that form their plasma
or liner in situ---plasma-jet MIF (the imploding liner is itself
plasma), the liquid-liner magnetized-target loop, pulsed FRC, theta
pinch, dense plasma focus, and sheared-flow / staged Z-pinch---set
$c_{\text{target}} = 0$ and carry $C_{220108} = 0$. The same knob
governs the target-factory power draw, so an in-situ concept incurs
neither the factory capital nor its recirculating power, with no cost
override required.
...
\textit{material that fills it}. The cost is a per-fuel base scaled
linearly with net electric power and multiplied by a chemistry factor
$\phi(\mathrm{fill}) \in \{0.0, 1.0, 2.0, 3.0, 5.0, 13.0\}$ selected by
the blanket fill chemistry:
\begin{equation}
\label{eq:cas27}
C_{27} = C_{\text{mat,base}}(f) \cdot \phi(\mathrm{fill}) \cdot \frac{P_{\text{net}}}{1000}
\end{equation}
...
material chemistry, driving $\phi(\mathrm{fill})$ here). Compatibility
between form and fill is enforced at validation time.

\Cref{tab:cas27_fills} lists the chemistry factors and their cost
basis, calibrated relative to the D-T PbLi baseline (4{,}000 tonnes of
Pb-17Li eutectic at \$3/kg plus enriched lithium premium, totaling
\$15M at 1\,GWe).

\begin{table}[h]
\centering
\caption{CAS27 fill chemistry factors $\phi(\mathrm{fill})$ relative to the D-T PbLi baseline (\$15M at 1\,GWe).}
\label{tab:cas27_fills}
\begin{tabular}{lrl}
\toprule
Fill chemistry & $\phi(\mathrm{fill})$ & Cost basis \\
\midrule
\texttt{pbli} (PbLi eutectic)        & 1.0  & Baseline: 4{,}000 t at \$3/kg + Li-6 premium \\
\texttt{li} (self-cooled Li)         & 2.0  & ~300 t at \$300--1000/kg enriched, center \$30M \\
\texttt{flibe} (2LiF-BeF$_2$)        & 5.0  & ~1{,}000 t at \$50--150/kg with Li-7 enrichment, ~\$75M \\
\texttt{be\_ceramic} (HCPB)          & 13.0 & ~300 t Be at \$500--800/kg + Li-ceramic pebbles, ~\$200M \\
\texttt{ceramic\_only} (WCCB)        & 3.0  & Li-ceramic pebbles without Be multiplier, ~\$45M \\
\texttt{none}                        & 0.0  & Aneutronic, no breeder \\
\bottomrule
\end{tabular}
\end{table}

The HCPB (\texttt{be\_ceramic}) factor of 13.0 is driven by global
beryllium supply: world production is ~300 tonnes/yr, so a single
DEMO-scale plant consumes the entire annual supply, which makes the
...
and it is the single knob that also switches on the
target factory $C_{220108}$. NOAK mass-production estimates span
\$0.25--0.70/shot for laser and heavy-ion capsules (where the
fabricated shell is a few percent of LCOE) up to several dollars per
shot for liner-plus-transmission-line loads, the transmission-line cost
being net of in-situ material recycling. Because the rep rate enters
...
contributes a multiple-percent share of LCOE, whereas a $\sim$0.1~Hz
liner concept is dominated by its per-unit hardware cost at far lower
annual throughput.
...
capital cost of \$2{,}660M, an overnight specific cost of
9{,}853~\$/kWe, and an LCOE of 137.5~\$/MWh under the framework's
NOAK default assumptions. The implied delta on overnight cost is
$-65$\,\%. Sorbom's volumetric estimate uses a single \$1.06M/tonne
...
% Direct: published 1521.1 M$ (1992) * 2.295 = 3491; 1cfe sum CAS21+22+23+24+25+26+27 = 3493
% Overnight $/kWe: published 2441 (1992) * 2.295 = 5602; 1cfe (CAS10-50)/kWe = 4586
% Total: published 2844 M$ (1992) * 2.295 = 6527; 1cfe total_capital = 5468
% LCOE: published 47.53 mill/kWh (1992) * 2.295 = 109.1 $/MWh; 1cfe = 77.3
\begin{table}[h]
\centering
...
\midrule
Direct cost (Account 90), M\$       & 3{,}491 & 3{,}493 &  $+0.1$ \\
Overnight specific cost, \$/kWe     & 5{,}602 & 4{,}586 & $-18.1$ \\
Total cost (Account 99), M\$        & 6{,}527 & 5{,}468 & $-16.2$ \\
LCOE, \$/MWh                        &   109.1 &    77.3 & $-29.2$ \\
\bottomrule
\end{tabular}
\end{table}
