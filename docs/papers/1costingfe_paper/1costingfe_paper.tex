...
CAS50        & Supplementary costs                          & 260.1 \\
CAS60        & Interest during construction                 & 628.5 \\
\midrule
\multicolumn{2}{l}{Total overnight}                          & 3898.6 \\
\multicolumn{2}{l}{Overnight specific cost (\$/kW)}          & 3899 \\
\midrule
CAS70 (M\$/yr) & O\&M and scheduled replacement             & 41.9 \\
CAS80 (M\$/yr) & Fuel (raw isotopes)                        & 250.5 \\
CAS90 (M\$/yr) & Annualized financial (CRF, \cref{eq:crf})   & 314.2 \\
\midrule
\multicolumn{2}{l}{\textbf{LCOE (\$/MWh)}}                  & \textbf{81.5} \\
\bottomrule
\end{tabular}
\end{table}
...
\citep{emwg2007}. The 10\% FOAK rate is conservative relative to the EMWG
...
superconducting-coil concepts. The per-fuel totals (each a sum of the
individually-costed buildings) are summarized in \cref{tab:cas21-fuel}.
...
The fusion p-${}^{11}$B figure of 384~\$/kW corresponds to roughly
...
$P_{\text{et,ref}}$ (\cref{sec:cas2206}). It originates from the ARIES-CS design study
...
capital cost of \$2{,}875M, an overnight specific cost of
10{,}650~\$/kWe, and an LCOE of 143.3~\$/MWh under the framework's
NOAK default assumptions. The implied delta on overnight cost is
$-62$\,\%. Sorbom's volumetric estimate uses a single \$1.06M/tonne
...
% Overnight $/kWe: published 2441 (1992) * 2.295 = 5602; 1cfe (CAS10-50)/kWe = 4765
% Total: published 2844 M$ (1992) * 2.295 = 6527; 1cfe total_capital = 5681
% LCOE: published 47.53 mill/kWh (1992) * 2.295 = 109.1 $/MWh; 1cfe = 76.7
\begin{table}[h]
\centering
...
Overnight specific cost, \$/kWe     & 5{,}602 & 4{,}765 & $-14.9$ \\
Total cost (Account 99), M\$        & 6{,}527 & 5{,}681 & $-13.0$ \\
LCOE, \$/MWh                        &   109.1 &    76.7 & $-29.7$ \\
\bottomrule
\end{tabular}
\end{table}

The direct-cost row matches closely (within 1\,\%), indicating
that the bottom-up CAS21--27 procurement basis agrees with the heritage 2002
ARIES systems-study estimate at the top level. The overnight specific
cost is 15\,\% lower; this is consistent with \texttt{1costingfe}'s
NOAK default assumption replacing the FOAK-with-learning-multiplier
convention used in the ARIES program. The LCOE divergence ($-30$\,\%)
...
cases. The principal compositional
