...
$B_{\min}$    & 3 T                 & central-cell midplane field (\texttt{B}); coil pricing derives from it (central solenoid at $B_{\min}$, plugs at $R_m B_{\min}$) \\
...
result = model.forward(
    ...
    chamber_length=80.0, plasma_t=0.4, B=3.0,
    T_e=70.0, n_e=3.3e19,
...
CAS22        & Reactor plant equipment                      & 1499.3 \\
CAS23--29    & Other direct (BOP, special materials, contingency) & 242.0 \\
CAS30        & Capitalized indirect services                & 413.1 \\
CAS40        & Owner's costs                                & 24.3 \\
CAS50        & Supplementary costs                          & 242.4 \\
\midrule
\multicolumn{2}{l}{Overnight capital (CAS10--50)}            & 2759.7 \\
\midrule
CAS60        & Interest during construction                 & 530.5 \\
\multicolumn{2}{l}{Total capital (incl.\ IDC)}               & 3290.2 \\
\multicolumn{2}{l}{Specific cost (\$/kW, incl.\ IDC)}        & 3290 \\
\midrule
CAS70 (M\$/yr) & O\&M and scheduled replacement             & 41.9 \\
CAS80 (M\$/yr) & Fuel (raw isotopes)                        & 250.5 \\
CAS90 (M\$/yr) & Annualized financial (CRF, \cref{eq:crf})   & 265.1 \\
\midrule
\multicolumn{2}{l}{\textbf{LCOE (\$/MWh)}}                  & \textbf{74.9} \\
\bottomrule
\end{tabular}
\end{table}
...
parameter is \texttt{fuel\_recovery} (elasticity $-7.65$), in the
physics-outputs band; the most consequential cost unit price is
\texttt{u\_he3}, the ${}^3$He purchase price ($+0.46$); and the most
consequential financial parameter is \texttt{interest\_rate} ($+0.37$).
...
elasticity of $+0.37$ and \texttt{inflation\_rate} $+0.17$.
...
result = model.forward(
    net_electric_mw=1000.0, availability=0.85, lifetime_yr=30,
    cost_overrides={"C220103": 140.0},  # M$, mirror coil quote
)
\end{lstlisting}
\end{minipage}
...
For laser IFE, Z-pinch, plasma jet, and other single-pass pulsed concepts in
which the energy leaving the plasma is absorbed as heat and converted through
a thermal cycle (rather than recovered electromagnetically, as in the
...
electromagnetically rather than as heat (the pulsed FRC, e.g.\ Helion),
the framework includes a dedicated inductive
...
\subsubsection{Pulsed Recovered-Compression Conversion}
A third class of pulsed concept compresses a magnetized target with a driver
whose energy is largely \emph{recovered} each cycle rather than consumed,
while the fusion output remains a neutron-dominated thermal load. For these,
the single per-pulse driver energy that suffices for a single-pass driver
splits into three physically distinct quantities: the energy \emph{delivered}
to the plasma $E_{\text{drv}}$, which sets the scientific gain; the peak
energy \emph{stored} in the pulsed-power system $E_{\text{store}}$, which sizes
the capacitor / energy-transfer capital; and the net energy \emph{consumed}
from the grid per pulse $E_{\text{rec}}$, which sets the recirculating load.
Only the dissipated fraction $(1-r)$ of the delivered energy thermalises,
with $r$ the recovery fraction:
\begin{equation}
P_{\text{th}} = M_n P_n + (1 - f_{\text{dec}})\,P_{\text{ch,net}}
  + P_{\text{rad}} + (1-r)\,P_{\text{drv}} + \eta_p\,P_{\text{pump}},
\end{equation}
the recirculating driver load is $E_{\text{rec}}\,f_{\text{rep}}$ (rather than
$P_{\text{drv}}/\eta_{\text{pin}}$), and the pulsed-power capital scales with
$E_{\text{store}}$. For a single-pass driver the three energies collapse
($E_{\text{store}} = E_{\text{drv}}/\eta_{\text{pin}}$,
$E_{\text{rec}} = E_{\text{drv}}(1-r)/\eta_{\text{pin}}$, $r = 0$), recovering
the pulsed-thermal balance above, so only the two concepts below set the split
explicitly.

The magnetized target (General Fusion-class) mechanically recovers its liner
kinetic energy: for the 150~MWe reactor design point of
\citet{krotez2023mtf}, the poster's stated constant
$1-\eta_{\text{mech}} = 15\%$ fixes $r = 0.85$, so only that dissipated
fraction of the 755~MJ/shot liner energy enters the heat pool, while the grid
draw and capacitor store are set by the plasma-injector and gas-compressor
systems rather than the full liner energy. The theta pinch instead recovers
its compression-field energy \emph{reactively}: the energy-transfer-and-storage
circuit swings the full field energy into the coils and rings roughly 98\% back
out each cycle, so for the Los Alamos Reference Theta-Pinch Reactor
\citep{burnett1972rtpr} the stored energy ($\sim$110~GJ/pulse) exceeds the net
consumption ($\sim$2~GJ/pulse) by nearly two orders of magnitude. That large
reactive store dominates the concept's pulsed-power capital, the quantified
expression of the long-standing economic difficulty of theta-pinch reactors.

%% ------------------------------------------------------------
\section{Sizing from a Power Target}
\label{sec:sizing}
The forward call evaluates a plant at a fixed operating point. To compare
concepts at a common scale, the framework also runs the other way: given a
target net electric power $P_{\text{net}}^{\text{tgt}}$, it solves for the plant
configuration that delivers it (\texttt{size\_from\_power}). Each concept has a
natural single-device scaling lever, and the model implements three of them ---
device volume, module replication, and repetition rate --- each solving one
variable so that the delivered net electric matches the target. A common
overflow rule handles targets that exceed what one device can deliver.

\subsection{Volume: The Tokamak Operating Point}
The tokamak scales by growing a single device, so its sizing variable is the
major radius $R_0$, and evaluating the net electric at a candidate $R_0$ is
itself a 0D operating-point solve. The minor radius follows from the aspect
ratio and the plasma cross-section from the elongation $\kappa$; the on-axis
toroidal field $B_0$ is set by the radial build together with a peak
field-on-coil limit $B_{\max}$, so a larger $R_0$ admits a thicker inboard leg
and thus a different achievable $B_0$. At that geometry and field the model
selects the temperature $T$ that \emph{maximizes} net electric subject to the
normalized-beta (Troyon) limit $\beta_N \le \beta_{N,\max}$, by golden-section
search over $T \in [T_{\min}, T_{\max}]$ with constraint violations penalized so
the optimum settles on the beta boundary. The energy confinement time is the
IPB98(y,2) scaling multiplied by a quality factor $H$,
$\tau_E = H\,\tau_E^{\text{IPB98}}$, which fixes the heating power needed to
sustain that point --- and hence the recirculating load --- rather than taking a
heating power as an input.

The resulting $P_{\text{net}}(R_0)$ is monotone increasing, so the target radius
is found by bisection on $[R_{0,\min}, R_{0,\max}]$. The upper bracket
$R_{0,\max}$ is a concept ceiling, and the net electric it delivers,
$P_{\text{net}}(R_{0,\max})$, is the most a single tokamak can produce; it feeds
the overflow rule below.

\subsection{Module Replication and Repetition Rate}
Two other levers require no geometry solve. Concepts that scale by replicating a
fixed module --- the orbitron, dense plasma focus, sheared-flow Z-pinch, and
steady FRC --- take an integer module count
\begin{equation}
n_{\text{mod}} = \left\lceil P_{\text{net}}^{\text{tgt}} / P_{\text{mod}}
  \right\rceil,
\end{equation}
where $P_{\text{mod}}$ is the per-module design net power; the realized plant
power overshoots the target by less than one module. Pulsed concepts instead
hold a cited per-shot design point --- delivered energy, fusion yield, and a
maximum rate $f_{\text{rep}}^{\max}$ --- fixed and solve the repetition rate
$f_{\text{rep}}$ from the target. Because both the fusion and the recirculating
powers scale with $f_{\text{rep}}$ while the fixed loads (housekeeping,
cryogenics) do not, $P_{\text{net}}(f_{\text{rep}})$ is monotone increasing and
$f_{\text{rep}}$ is found by bisection up to $f_{\text{rep}}^{\max}$.

\subsection{The Unit-Ceiling Overflow Rule}
All three axes share one rule when the target exceeds a single device. Each has a
per-unit ceiling --- the most one device can deliver: $P_{\text{net}}(R_{0,\max})$
for volume, $P_{\text{mod}}$ for a module, and $P_{\text{net}}(f_{\text{rep}}^{\max})$
for a rep-rated chamber. When the target exceeds it, the model builds the integer
number of identical units required,
$n = \lceil P_{\text{net}}^{\text{tgt}} / P_{\text{unit}} \rceil$, and solves each
unit to $P_{\text{net}}^{\text{tgt}} / n$. A fractional device cannot be built, so
the realized plant power overshoots the request by less than one unit, and $n$ is a
physical integer returned on the result rather than supplied by the user. This
makes the rep-rate ceiling economically meaningful: a chamber that cannot reach
the target even at $f_{\text{rep}}^{\max}$ adds chambers rather than demanding an
unphysically fast pulse. Sizing is enabled for the tokamak, the four
module-replication concepts, and the five rep-rate concepts; the remaining
concepts carry the sizing law but are not yet switched on.
...
$s(\mathrm{form}) \in \{1.0, 1.3, 1.2, 1.0, 0.0\}$ for liquid-metal,
molten-salt, solid-breeder, water-cooled, and no-blanket architectures. The unit cost
is \emph{structure only} (RAFM steel, tungsten first-wall armor, and
fabrication); the breeder and neutron-multiplier fill it holds is priced
separately under CAS27 (\cref{sec:cas27}). The D-T unit cost is anchored to a
structure-only mass build-up at the reference machine (about 1300~t of
fabricated nuclear-grade steel plus tungsten armor over the roughly
640~m$^3$ blanket, cross-checked against the 440-module ITER
blanket/shield) and priced at mature, Nth-of-a-kind unit costs:
\begin{equation}
\label{eq:cas2201}
...
For aneutronic machines with no blanket at all ($s(\mathrm{none}) = 0$, the
default fuel normalization for D-${}^3$He and p-${}^{11}$B), the
plasma-facing wall is priced instead as surface hardware,
\begin{equation}
\label{eq:cas2201fw}
C_{22.01.01} = A_{\text{fw}} \cdot c_{\text{fw}}(\text{class}),
\end{equation}
with three discrete hardware classes declared by the design (the
\texttt{fw\_class} input, symmetric with \texttt{coil\_material}): an
actively-cooled panel wall ($0.35$~M\$/m$^2$ NOAK, qualified to about
2~MW/m$^2$ steady state; ITER first-wall-panel procurement anchor), a
divertor-grade monoblock wall ($2.5$~M\$/m$^2$ NOAK, qualified to
10~MW/m$^2$; ITER divertor procurement anchor), and a beam-dump-grade
bare-CuCrZr wall ($1.0$~M\$/m$^2$ NOAK, rated to 20~MW/m$^2$ through-wall
in the ITER NBI calorimeter/RID class). The beam-dump class has no public
procurement anchor (the relevant awards disclose supplier and scope but not
value), so its unit cost is a mass build-up: about 0.9~t/m$^2$ of
fabricated CuCrZr at qualified-copper rates, doubled for manifolds,
integration, and QA. It is cheaper than the monoblock class despite the
higher rating because it is a homogeneous copper-alloy panel with no
tungsten armor or armor-to-heat-sink joining. The declared class is
audited against the machine's surface-flux cap. Discrete classes mirror the
hardware reality: qualified wall products exist as panels,
high-heat-flux components, and beam-stopping elements, and within a class
the cost is area-driven.

The fuel-keyed unit cost $c_{\text{blk}}(f)$ is $0.35$, $0.30$, $0.08$,
and $0.05$~M\$/m$^3$ for D-T, D-D, D-${}^3$He, and p-${}^{11}$B, declining
from a full D-T breeding-blanket structure (steel structure, tungsten armor,
tritium-barrier coatings, and the fabrication of a flow loop and MHD ducts) to
a thin X-ray-capture wall with no breeding (p-${}^{11}$B). The D-T and D-D
values sit close together because both price the same steel structure; their
gap is the D-T W-armor and tritium-barrier premium, not the breeder or
multiplier, which are in CAS27. A solid Li$_2$O ceramic breeder (used by the
levitated dipole) substitutes a dedicated structure base
$c_{\text{Li}_2\text{O}} = 0.35$~M\$/m$^3$ in place of $c_{\text{blk}}(f)$, so
its breeding-blanket structure is priced on the same steel-and-armor basis as
D-T rather than off the fuel key.
...
by default) priced at the central-cell midplane field $B_{\min}$ they
produce on axis, plus $n_{\text{plug}} = 4$ high-field end-plug coils at
the throat field $R_m B_{\min}$, where $R_m$ is the mirror ratio (throat
field over midplane field). Both classes derive their pricing field from
the single stored plasma field; no concept stores a second, independent
coil field that could drift from the design point. Each class carries its
own markup (\cref{tab:cas2203-markup}). The geometry factor
$G = (n_{\text{central}} + n_{\text{plug}}) \cdot 4\pi$
...
\midrule
Tokamak     &  3.0$\times$ & 1.0 & \$500M & TF+CS+PF complexity; derived from the ARC magnet BOM \citep{sorbom2015arc} with case steel at the ITER-FDR qualified-magnet-structure rate (\$50/kg): BOM/conductor $=2.9$, closed to 3.0 by FDR-class winding content \citep{huguet2001iter} \\
Stellarator &  5.7$\times$ & 2.0 & \$2{,}068M & Non-planar 3D coils; $1.9\times$ the tokamak markup, the {NCSX} modular-coil production cost overrun \citep{neilson2010ncsx} \\
Mirror      &  1.52$\times$ / 1.81$\times$ & 1.0 & \$164M & Two-class: $n_{\text{central}}=L/d_{\text{coil}}$ central solenoids at $B_{\min}$ with bore $r_{\text{coil}} = r_{\text{vessel,out}} + 0.10$~m (markup 1.52$\times$, the PF-coil class of the ITER FDR magnet cost breakdown \citep{huguet2001iter}) plus $n_{\text{plug}}=4$ plug coils at the throat field $R_m B_{\min}$ with bore $r_{\text{coil}} = a/\sqrt{R_m} + 0.30$~m (markup 1.81$\times$, the CS class of the same breakdown) \\
...
\end{equation}
where $c_{\text{NBI}} = 7.5$, $c_{\text{ICRF}} = 1.0$,
$c_{\text{ECRH}} = 5.3$, $c_{\text{LHCD}} = 1.0$ (all M\$/MW).
The NBI and ECRH anchors are procurement bands, quoted to two
significant figures: NBI is the midpoint of the ITER NBI procurement
band (EUR~9--15M/MW FOAK) after a 30--50\% NOAK learning discount, and
ECRH is mid-band ITER gyrotron procurement (EUR~5--10M/MW). ICRF and
LHCD have no procurement anchor of comparable quality: no public
\$/MW figure for ion-cyclotron or lower-hybrid heating hardware
exists, and the 1.0 value is a NOAK point estimate from private
communication, carried without a range. They carry the same value
because they are the same RF-source hardware class, tetrode
transmitters and klystrons with no beamline and no superconducting
content. Read at matching scope, the nearest published analog (the
klystron plus auxiliary RF systems of a published 1~MW
superconducting-linac design study) prices at about \$3.2/W, so the
value here embeds an aggressive NOAK assumption, roughly $3\times$
below the published analog.
...
$S_{\text{req}} = Q_{\text{gas}}/P_{\text{op}}$. The gas load has two
sources; wall outgassing scales with surface area but is negligible for
a baked UHV system and is omitted:
\begin{align}
  Q_{\text{NBI}} &= g\, \frac{P_{\text{NBI}}}{E_b}\, k_B T, \\
  Q_{\text{fuel}} &= \left[ \mu(f)\, \frac{1 - f_b}{f_b} + a(f) \right] \frac{P_{\text{fus}}}{E_{\text{fus}}}\, k_B T,
\end{align}
...
load scales as $1/E_b$), and $E_{\text{fus}}$ the fuel-dependent energy
per reaction. The pump removes gas-phase particles, so the species
factors are fuel-keyed: $\mu(f)$ counts gas particles per unburned
consumed-ion-pair (hydrogenic atoms recombine to diatomic molecules;
condensable species drop out --- unburned boron is a refractory solid
handled as a liner solids loop, not pumping speed), and $a(f)$ counts
gas-phase ash per reaction (helium does not condense and must be
pumped). $\mu = \{1.0, 1.0, 1.5, 0.5\}$ and $a = \{1.0, 0.75, 1.5,
3.0\}$ for D-T, D-D, D-$^3$He, p-$^{11}$B respectively. The pumping
cost is then
\begin{equation}
\label{eq:cas2206-pump}
C_{\text{pump}} = c_{\text{pump}}(\text{basis})\,\frac{Q_{\text{NBI}} + Q_{\text{fuel}}}{P_{\text{op}}},
\end{equation}
where the unit cost depends on the concept's pump-technology basis:
\$15/(L/s) for discrete valved/ducted pumps behind ports (closed
toroidal machines and enclosed pulsed chambers; ITER's tritium-rated
FOAK torus cryopumps bracket this from above at roughly \$50/(L/s)),
versus \$2.5/(L/s) for bare cryopanel arrays lining the exhaust tanks of
open-geometry machines (mirror, FRC, dipole, cusp devices, and
Helion-class pulsed FRC, whose patents route the exhaust along open
field lines to remote divertor chambers). The cryopanel figure is the
center of four independently costed mirror/DEC designs escalated to
2025 USD (Hoffman UCID-17560; WITAMIR-I UWFDM-400; MARS UCRL-53480;
TASKA KfK-3311), and the technology is demonstrated at scale: MFTF-B
built about 1{,}100~m$^2$ of cryopanels, still the largest such system
ever constructed.
...
losses. The fueling term dominates over the NBI term at reactor beam
energies. Aneutronic p-B$^{11}$ runs about twice the reaction rate of
D-T at equal fusion power (8.7 versus 17.6~MeV per reaction), but its
condensable boron and molecular hydrogen pumping nearly cancel that
factor in gas terms (12.5 pumped particles per reaction versus D-T's
20). The pumping model applies uniformly to every concept, so these
differences follow from the inputs rather than per-concept manual
adjustment.
...
The lifetime requirement saturates at the plant's own shot count: a
1~Hz plant over 30 years at 85\% availability fires about $8\times10^8$
shots, so bank life beyond roughly $10^9$ shots outlives the plant and
buys nothing. The gap to close is to the plant's shot count, not
beyond it. Because a bank below that threshold is replaced on a
schedule the user may not have noticed, the model warns whenever
\texttt{cap\_shot\_lifetime} falls short of the plant's shot count,
reporting the implied replacement count. The NOAK life target is far
from demonstrated practice: laboratory pulse capacitors are rated near
$1.7\times10^4$ shots, and buying life by voltage derating costs
volume, since measured metallized-polypropylene life scales as
$(E/E_0)^{-7.32}$ while energy density scales as $E^2$, so a
$1000\times$ life extension carries roughly a $6.6\times$ cost penalty
before any manufacturing cost reduction is applied.
...
CAS22.01.09 covers direct energy conversion hardware in two branches:
the electrostatic venetian-blind add-on for steady-state linear devices
...
as electricity, and circuit-derived markups for pulsed inductive
concepts, where the compression coils and driver bank are themselves the
recovery hardware (both branches are booked to this account).

\paragraph{Electrostatic (venetian blind)}
...
\paragraph{Pulsed inductive DEC}
A pulsed inductive concept has no add-on converter: the compression
coils and the capacitor bank that drives them are also the recovery
hardware, so the account prices incremental markups on the driver store
$C_{220107}$ (\cref{sec:cas2207}) rather than standalone equipment:
\begin{equation}
\label{eq:cas2209-inductive}
  C_{220109} = C_{220107}\left[\max(m_{\text{cap}},0)\,k_{\text{st}}
  + m_{\text{sw}} + m_{\text{ctrl}}\right]
  + c_{\text{inv}} P_{\text{net}},
\end{equation}
where $m_{\text{cap}} = \eta_{\text{dec}}(1 + Q_{\text{sci}}
f_{\text{ch}}) - 1$ sizes the incremental store for the energy that
comes back each pulse (driver energy plus charged fusion yield, less
conversion losses, minus the one unit already bought as $C_{220107}$),
$m_{\text{sw}} = 0.06$ is the bidirectional-switch premium,
$m_{\text{ctrl}} = 0.04$ covers energy-management upgrades, and
$c_{\text{inv}} = \$150$/kW is the grid-tie inverter. The factor
$k_{\text{st}}$ (\texttt{dec\_staged\_recovery\_factor}, default 1.0)
is a capital-only credit for staged recovery topologies that avoid
rating a single capacitor for full voltage reversal; vendor patent
literature describes the two-stage arrangement as cheaper for the same
duty but publishes no figure, so no reduction is assumed and the knob
is exposed for sensitivity sweeps. It multiplies only the incremental
capital term; the recovered energy itself is credited in the power
balance, never here. Because the returned energy carries the fusion
yield, the store term grows linearly with $Q_{\text{sci}}$ while
per-module fusion power grows with it too, so at large gain the account
approaches a fixed cost per unit of converted power rather than falling
with gain: LCOE is not monotone in gain for these concepts, and raising
target gain is not automatically economic.
...
The primary-coolant row is the exception: its coefficient is expressed on
a thermal-power basis (a coolant loop is sized by the heat it moves, not
the electricity sold) and is consistent with the \$150--250M primary
coolant system of a 1~GWe (3000~MWt) PWR, so it stands on a fission
comparator rather than on the inherited reference plant.
...
\midrule
CAS22.02 & Primary coolant & 166 & $P_{\text{th}}/3000$ & 1.0 \\
...
through the chosen fill, which is normalized by fuel so a concept's
D-T-flavored default is not carried onto a fuel that cannot use it:
aneutronic fuels (D-He$^3$, p-$^{11}$B) use \texttt{none} and recover
zero, while D-D uses \texttt{water} --- it breeds its own tritium
(D+D$\rightarrow$T+p) so it needs no lithium breeder, but is neutronic
and so keeps a low-$Z$ moderating energy-capture blanket whose inventory
cost is negligible. \Cref{tab:cas27_fills} lists the per-fill inputs.
...
\texttt{water}         & 1000 & 0.50 & 0.01 & Light-water moderator, no breeder (D-D) \\
...
Pulsed concepts add driver-specific consumables. Every pulsed-power
concept's capacitor bank is replaced on a cycle-count and repetition-rate
schedule, independent of the DEC conversion path, since the bank is the
least lifetime-proven driver component. At the NOAK cap lifetime
($10^{8}$ shots, conservative against the $\sim\!10^{9}$-shot target of
derated designs) a low-repetition-rate bank outlives the plant, giving a
correct zero whose driver-lifetime cost is instead carried in the capital
\$/J basis. The electromagnetic-gun concepts (sheared-flow Z-pinch and plasma jet)
...
\texttt{1costingfe} predicts a \$5{,}212M capital cost (19{,}302~\$/kWe)
and a 236.1~\$/MWh LCOE, a $-31$\,\% delta.
...
4{,}350-tonne SS316LN magnet case structure priced at \$4.6\,B (FY2014~USD,
\$6.3\,B escalated) --- a single line that by itself exceeds
\texttt{1costingfe}'s entire \$5.2\,B estimate, even with the coils at the
current REBCO price (\$1.53B, 29\,\% of capital). \texttt{1costingfe}
carries that same steel inside the C220103 coil markup
(\cref{tab:cas2203-markup}), where the ARC magnet bill of materials is
priced with the case steel at the ITER-FDR qualified-magnet-structure rate,
about \$50/kg in 2025~USD: load-bearing, NDT-inspected, cryogenic-qualified
316LN, priced as such --- several times commodity welded plate, but far
below the heritage multiplier. The two estimates price the same steel on
different bases: \$1{,}060/kg is a fabricated-fusion-component figure
carried over from one-off research machines that bundles design,
instrumentation, and assembly into the steel mass. That factor of roughly
20 (about 29 after escalating Sorbom to 2025) is a
heritage-versus-qualified-fabrication pricing choice for magnet structure,
not a model error on either side.
...
% Direct: published 1521.1 M$ (1992) * 2.295 = 3491; 1cfe sum CAS21+22+23+24+25+26+27 = 3345
% Overnight $/kWe: published 2441 (1992) * 2.295 = 5602; 1cfe (CAS10-50)/kWe = 5035
% Total: published 2844 M$ (1992) * 2.295 = 6527; 1cfe total_capital = 6002
% LCOE: published 47.53 mill/kWh (1992) * 2.295 = 109.1 $/MWh; 1cfe = 79.4
\begin{table}[h]
\centering
...
\midrule
Direct cost (Account 90), M\$       & 3{,}491 & 3{,}345 &  $-4.2$ \\
Overnight specific cost, \$/kWe     & 5{,}602 & 5{,}035 &  $-10.1$ \\
Total cost (Account 99), M\$        & 6{,}527 & 6{,}002 &  $-8.0$ \\
LCOE, \$/MWh                        &   109.1 &    79.4 & $-27.2$ \\
\bottomrule
\end{tabular}
\end{table}

Run at FOAK to match the ARIES program's convention, \texttt{1costingfe}
reproduces the published capital within roughly 10\,\% on the low side:
direct cost within 5\,\% ($-4.2$\,\%), overnight specific cost within
11\,\% ($-10.1$\,\%), and total cost within 9\,\% ($-8.0$\,\%). The residual
LCOE divergence ($-27$\,\%)
