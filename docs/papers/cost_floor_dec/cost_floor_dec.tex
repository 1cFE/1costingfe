...
1\,GWe, falling to \$9.5/MWh only at an extreme limit of 5\,GWe
...
\midrule
Turbine and generator & \$181M & sCO\textsubscript{2}
...
tritium containment barriers, and nuclear-grade HVAC with HEPA
filtration. A D-He3 plant retains some radiation shielding
...
The parameters that lower the floor are
...
plant scale and financial parameters, with a free fusion core. Core
budget = \$10/MWh target minus floor; negative values indicate the
floor alone exceeds the one-cent target.}
\label{tab:dt-sweep}
\centering
\begin{tabular}{lrrr}
\toprule
...
budget (\$/MWh) \\
\midrule
Baseline: 1\,GWe, 85\%, 7\%, 30\,yr, 6\,yr & 29 & 1{,}723 & $-19$ \\
...
approximately 3\% of its primary-reaction energy as neutrons via
D-D side reactions, requiring some shielding and modest tritium
monitoring; buildings amount to \$418M and the BOP-only floor at
1\,GWe is \$19/MWh. The D-He3 plant carries a separate burden:
at an optimistic helium-3 price of \$2M/kg, fuel contributes
\$46/MWh to LCOE \citep{Shea2010}; the DOE-allocated price of
approximately \$4.5M/kg more than doubles this. Helium-3 is not
...
fusion energy emerges as charged alpha particles.
...
\$1{,}098/kW of overnight capital). This figure sits squarely
...
extreme limit (5\,GWe D-T), O\&M contributes \$5.2/MWh of the
\$9.5/MWh floor; the capital-only component is \$4.3/MWh.
Reducing D-T staffing from 78 to roughly 42 FTE (54\% of current
levels) through SCADA-automated grid dispatch, reduced shift
coverage, and streamlined radiation protection procedures (each
realistic with current industrial automation) lowers the 2\,GWe
aggressive D-T floor to \$10/MWh, although still leaving
negligible budget for the costed core. Lights-out operation, in the sense of a near-zero-FTE plant,
...
\subsection{Implications of the Industrial Cost Floor}
...
An alternative is direct energy conversion (DEC), a class of conversion mechanisms
which is less mature but has some enticing potential advantages.
The remainder of
this paper examines whether that alternative actually lowers the
floor characterized in Section~\ref{sec:floor}, and where its
leverage lies if not.

\section{Direct Energy Conversion Architectures}
\label{sec:dec}

Direct energy conversion (DEC) is used here as an umbrella term for
any architecture that bypasses the thermal cycle, the turbine, or
both. The thermal default has two drawbacks: physics caps its
efficiency through the Carnot limit imposed by first-wall and
blanket material temperature constraints, and its hardware is expensive.
Magnetohydrodynamic (MHD) generators bypass the turbine but retain
the heat cycle; charged-particle DEC bypasses both, converting the
kinetic energy of plasma particles directly to current by
decelerating them against an electrostatic potential or by inducing
current in surrounding coils as the plasma expands.

The thermodynamic motivation for charged-particle DEC follows from
the first drawback: charged particles emerge from fusion plasmas at
$10^8$--$10^9$\,K, so the Carnot ceiling
\[
...
The energy leaving the fusing plasma partitions among three channels:
neutrons, charged particles, and photons. The nuclear reaction itself
partitions output between charged products (alphas, protons) and
neutrons by reaction kinematics: D-T deposits
80\% of its energy in 14.1\,MeV neutrons; p-B11 is
aneutronic. Separately, photon emission is determined by 
plasma dynamics: bremsstrahlung from electrons scattering
off ions and synchrotron emission from electrons spiraling in a magnetic
field. The bulk of photon energy is bremsstrahlung, which scales with
electron temperature and with the square of the ion charge. Fuel
choice and operating point together determine the resulting
three-way split among neutrons, charged particles, and
bremsstrahlung (Figure~\ref{fig:channels}).

For p-B11, the high plasma temperature ($>$100\,keV) and Z=5 
boron drive intense bremsstrahlung:
...
D-He3 sits between the two: a nuclear-aneutronic primary reaction
with a few-percent neutron channel from D-D side reactions, and
bremsstrahlung at Z=2 carrying a share that depends on the D/He-3
mix and operating point. Two representative cases are shown in
Figure~\ref{fig:channels}. A 50/50 D/He-3 steady-state plasma at
$T = 70$\,keV (the canonical magnetic-confinement reference) holds
the bred tritium long enough to fuse with deuterium, opening a
14\,MeV-neutron channel that adds to the D-D neutron loss. A
deuterium-rich pulsed plasmoid ($n_D/n_\mathrm{He3} = 3.2$ at
$T = 100$\,keV; the Helion-like operating point) confines the
plasma too briefly for the secondary D-T reaction, exhausts the
unburned tritium, and eliminates the 14\,MeV channel, routing 81\%
of the fusion energy into charged particles.
...
fusion power (1{,}959\,MW) is re-radiated as bremsstrahlung;
17\% (401\,MW) escapes as charged-particle transport; both feed a
...
the DEC arithmetic. The 14.1\,MeV neutron flux additionally damages
any charged-particle DEC hardware in its line of sight: high-voltage
metallic grids (such as the venetian-blind collector) and the
associated electronics are far less neutron-tolerant than a turbine,
further weakening the DEC case for D-T. D-T is therefore treated as a thermal fuel in
this analysis and is not considered further in this section.

D-He3 is the most favorable charged-particle DEC fuel: the energy
split described above leaves up to 81\% of the fusion energy in the
charged-particle channel at the deuterium-rich pulsed operating
point \citep{Wesson2004}, and the modest neutron fraction from D-D
side reactions does not impose the activation, damage, and
tritium-handling burden of D-T.
...
90\% of the 17\% charged-particle margin (389\,MW).}
\label{fig:pB11-VB}
\end{figure}
...
bottoming): 1{,}379\,MW of charged-particle transport drives the
pulsed inductive DEC at 85\%; bremsstrahlung (276\,MW), neutrons
(40\,MW), and DEC waste are dumped to cooling. Total fusion power
$P_\mathrm{fus} = 1{,}696$\,MW.}
...
(\$/MWh) \\
\midrule
Thermal only (sCO\textsubscript{2} Brayton, 47\%) & 18 & 44 & 61 \\
Pulsed inductive (85\% $\eta$, no turbine) & 23 & 29 & 52 \\
\bottomrule
\end{tabular}
\end{table}

The BOP floor is \$5/MWh higher than the thermal baseline
(Table~\ref{tab:pulsed}). The turbine island is not the dominant
cost in the thermal plant; buildings (\$387M), the electrical
plant (\$96M), the operations and maintenance budget
(\$35M per year), and financing charges together exceed it.
Eliminating the turbine and synchronous generator removes
\$149M of overnight capital, but the \$593M
pulsed-power chain that replaces it more than offsets the savings.

The fuel cost is a separate concern. At \$2M/kg, helium-3
contributes \$29/MWh to LCOE for the inductive architecture; at
the DOE-allocated price of \$4.5M/kg, this more than doubles.
...
\$23/MWh, still above the thermal baseline (\$18/MWh) and above
the one-cent target.
...
Lorentz force on a conducting fluid, with no moving parts. The MHD
generator is not a charged-particle DEC: it replaces the Brayton
turbine, not the heat cycle, and remains Carnot-limited. For
aneutronic fuels it could handle the bremsstrahlung fraction that
charged-particle DEC cannot touch. Coal-combustion MHD generators
reached TRL 5--6 through national programs in the 1960s--1990s;
liquid-metal MHD generators (LMMHD) have separately shown $>$71\%
efficiency at small scale \citep{Fabris1981}, and projected
efficiency for LMMHD on fusion coolants exceeds 60\%
\citep{Smolentsev2022}.

Fusion-coupled MHD generators have a costed paper-design heritage. The Compact Fusion
...
The remaining obstacle is that no fusion-coupled MHD generator has
ever been built. The electrode-erosion problem that ended the coal programs
...
efficiency insensitive to blanket pressure drops up to 50\%.
Fusion-coupled MHD generators therefore remain a what-if at the
hardware level (no operational demonstration).
...
inductive scheme then ends up within \$0.3/MWh of thermal, the pulsed-power
chain costing about as much as the turbine it replaces.

\begin{table}[htbp]
\caption{Conversion architectures under the aggressive scenario
(see Section~\ref{sec:dt}). Aggressive financing reduces the
BOP-floor contribution; the helium-3 fuel cost at \$2M/kg is
independent of plant financing.}
\label{tab:agg}
\centering
\begin{tabular}{lrrr}
\toprule
Approach & p-B11 (\$/MWh) & D-He3, \$2M/kg (\$/MWh) & D-He3, self-bred (\$/MWh) \\
\midrule
Thermal only (sCO\textsubscript{2}, 47\%) & 7.1 & 57 & 7.4 \\
VB DEC 60\% + thermal (hybrid) & 7.3 & 52 & 7.7 \\
Pulsed inductive (85\%, no turbine) & n/a & 41 & 7.7 \\
\bottomrule
\end{tabular}
\end{table}
...
LCOE from \$87/MWh to \$81/MWh, with a modest \$31M
increase in core cost partially offset by reduced heating-system
capacity. The pulsed inductive scheme lowers LCOE to \$74/MWh,
...
costs of \$1{,}538M and \$1{,}571M respectively. The DEC overlay
...
The floor of fusion electricity cost, computed with the core
assigned zero capital cost, is itself substantial. It is set by the
industrial fabric around the core (buildings, balance-of-plant,
staffing, and financing) and ranges from \$17/MWh (p-B11) to
\$29/MWh (D-T) at baseline; the fusion core adds further capital
cost on top. Within the floor, fuel choice dominates. BOP
equipment is essentially fuel-invariant (\$386--393M across all
three fuels when DEC is excluded), while buildings and staffing
carry the radioactivity and tritium-handling load. The \$215M
buildings gap between D-T and p-B11 at 1\,GWe exceeds most
published core-cost-reduction scenarios, and it propagates to a
\$5--8/MWh floor difference that compounds with financing
assumptions. Only p-B11, at aggressive scale and financing,
leaves meaningful budget for the costed fusion core within the
one-cent target.

Under current cost assumptions, direct energy conversion does not
lower this floor. For p-B11,
bremsstrahlung carries 83\% of the fusion power; even an efficient
venetian-blind DEC captures only the charged-particle margin and
adds hardware cost comparable to the turbine savings, leaving the
floor unchanged. For D-He3, an inductive scheme can route 81\% of
fusion energy through the recovery coils and reduce both required
fusion power and external helium-3 burn, but the pulsed-power chain
that replaces the turbine costs more than a turbine, so the BOP
floor rises. DEC's economic leverage is therefore exerted on the
costed core (via smaller required fusion power) and on the fuel
bill (via reduced helium-3 burn for D-He3), not on the industrial
floor. Three experimental milestones would convert this opportunity
into evidence.
...
The authors thank Robert Campbell, Mallory Snowden, Ian Ochs, Elijah Kolmes, and
