...
![Power flow in a 1 GWe p-B11 thermal-only mirror: 83% of fusion power (1,959 MW) is radiated as bremsstrahlung, 17% (401 MW) escapes as charged-particle transport, and both feed a single sCO2 turbine at 47%.](<p-B11 Thermal Cycle.svg>)
...
MHD generator is not a charged-particle DEC; it replaces the Brayton turbine, not the heat cycle, and is still Carnot-limited. Any fusion plant could route its coolant through an MHD channel instead of a turbine. For D-T, LMMHD integrated into the lithium breeding blanket handles neutron and thermal energy. For aneutronic fuels, it handles the bremsstrahlung fraction that charged-particle DEC cannot touch.

Fusion MHD has costed design heritage. The Compact Fusion Advanced Rankine (CFAR) concept, developed in the late 1980s by an LLNL/Kyoto/TIT collaboration ([Hoffman, Campbell, & Logan, 1989](https://www.osti.gov/biblio/6276900); [CFAR-II inertial variant](https://www.sciencedirect.com/science/article/abs/pii/092037969390114W)), pairs a high-temperature mercury Rankine loop and microwave superheater with a nonequilibrium MHD disk generator, packaged to fit almost entirely inside the reactor vault. The MFE variant reported 24-30% net plant efficiency at 2500-3250 K generator inlet; CFAR-II, with a vaporized blanket at 10⁴ K, projected 38%. Both require bespoke blankets at or past current materials limits, and neither was built. CFAR-II carries a costed 22-subsystem BoP model with direct cost of 30-40 M$/GWe (1992 dollars), the most explicit fusion-MHD BoP cost reference on record, though that figure does not price the blanket development gap, and the projected efficiencies sit below the >60% modern LMMHD target.

Folded into this post's framework, in-vault integration could trim on the order of $200M of BoP and buildings capex at 1 GWe. CFAR-II's 30-40 M$/GWe BoP (1992$) inflates to about $70-90M in 2024 dollars, against the blog's $168M sCO2 turbine island; net conversion-swap saving is $80-100M, with the rest from turbine-hall and piping, for a total LCOE saving of roughly $3/MWh, assuming blanket costs are the same. This may understate CFAR's intended impact. The original motivation was that a fusion core paired with a conventional BoP cannot reach the cost per MWh of fission, gas, or coal because the core is intrinsically expensive, so drastically shrinking the BoP is the path to competitiveness, not a marginal optimization on an already-competitive plant.

The remaining obstacle is that no fusion-MHD system has ever been built. The electrode-erosion problem that killed the coal programs doesn't apply to clean fusion coolants. For LMMHD integrated into a breeding blanket (the modern [ORNL and KIT line of work](https://doi.org/10.1080/15361055.2022.2116905)), the pressure drop of liquid metal flowing through fusion-scale magnetic fields (5-15 T) may exceed the generated power, and no reactor-scale demonstration exists. CFAR avoids this by topology: its disk generator sits outside the toroidal field coils in a region where the field has dropped off rapidly, with dedicated ring magnets, and the 1989 design study reported cycle efficiency insensitive to blanket pressure drops up to 50%. MHD therefore remains a what-if at the hardware level (no operational fusion demonstration).
...
Flipping the analysis: at fixed VB hardware cost, what efficiency would justify adding the DEC? At the 1 GWe p-B11 baseline the VB outputs roughly 210 MWe, from 90% capture of the 389 MW charged-particle margin at 60% conversion, and the model scales the VB hardware to about $85M at this plant scale. Holding that fixed, no VB efficiency between 30% and 100% breaks even with thermal-only; the VB hardware exceeds turbine-side savings across the entire physically accessible band. Break-even enters the physically accessible window only if the VB capex at this plant scale falls by roughly 3x, to around $30M, at which point a 54% VB suffices. The leverage is narrow: only 17% of fusion power flows through the DEC, so each percentage point of VB efficiency moves only 0.17 percentage points of plant-level conversion.

![Power flow in a 1 GWe p-B11 mirror with a venetian-blind DEC at 60% hybrid with a thermal cycle: 83% of fusion power (1,901 MW) radiates as bremsstrahlung and feeds the turbine, while the DEC captures only the 17% charged-particle margin (389 MW).](<p-B11 VB DEC.svg>)
...
The BOP floor comes in $5/MWh above thermal: $23/MWh vs $18/MWh. The turbine island is not the dominant cost. Buildings ($387M), the electrical plant ($96M), O&M ($35M/yr), and financing charges together dwarf the turbine, and the pulsed-power chain that replaces it costs more than the turbine. Eliminating the turbine and its synchronous generator (including the GSU transformer and sync/protection gear) removes about $149M of overnight capital, but the $593M of pulsed-power hardware needed to shape, store, and grid-tie the recovered energy more than offsets those savings.

The He-3 fuel cost is a different problem. At $2M/kg (optimistic), He-3 adds $29/MWh, nearly three times the 1-cent target by itself. At the [DOE-allocated price](https://www.everycrsreport.com/reports/R41419.html) of $4.5M/kg, it roughly doubles. Helion's answer is to run a D-rich mix that maximizes D-D-bred He-3 and recover the He-3 between shots at assumed 99% efficiency; tritium from D-D is exhausted (or held until decay) rather than re-burned. If the fuel cost drops to zero, the D-He3 pulsed DEC floor sits at $23/MWh, still above thermal ($18/MWh) and well above the 1-cent target.

Running the Helion-likely D-rich mix saves about $20/MWh of LCOE for pulsed inductive over the textbook 50/50 reference, almost entirely through reduced He-3 burn: more D-D side reactions produce more internal He-3 that the recovery loop re-injects, lowering external He-3 demand by about a quarter. Mix selection moves less than architecture choice (which moves $13/MWh between thermal and pulsed inductive), but it is a substantial lever at He-3 prices of $2M/kg.
...
How much cheaper does the pulsed-power hardware need to be for Helion to compete on price? Assuming Helion's stated goal of self-bred He-3 (fuel cost dropping to roughly zero), the Helion-likely LCOE falls from $74/MWh to about $45/MWh, comparable to advanced nuclear and at the lower end of natural gas combined-cycle ($50-70/MWh). The pulsed-power chain at the modeled $593M contributes about $10/MWh of that. Making it free saves $10/MWh and brings Helion to $35/MWh, below p-B11 thermal ($42/MWh). The floor below that is set by buildings, electrical plant, O&M, and financing, not the cap bank; closing to the 1-cent target is out of reach by hardware learning alone.

![Power flow in a 1 GWe D-He3 plant at the Helion-likely operating point (D-rich mix, T = 100 keV, T exhausted, 99% inter-shot He-3 recovery, no thermal bottoming): 1,379 MW of charged-particle transport drives the pulsed inductive DEC at 85%; bremsstrahlung (276 MW), neutrons (40 MW), and DEC waste are dumped to cooling. p_fus = 1,696 MW. The textbook 50/50 reference (24% brem, 3% neutrons) appears as the D-He3 column in the energy-channel figure above.](<Helion Pulsed Inductive.svg>)
...
For the venetian blind, the core cost rises modestly ($31M): the added DEC hardware partially offsets the savings from a smaller heating system. For pulsed inductive, the core jumps about $391M because the pulsed-power chain that replaces the turbine (dominated by the recovery cap bank) is substantial. LCOE drops $6/MWh with the venetian blind and $13/MWh with pulsed inductive, but the pulsed savings come mostly from fuel: higher efficiency, the D-rich operating mix, and active He-3 recovery between shots all reduce external He-3 demand. For D-He3, DEC is primarily a fuel-saving technology.

### p-B11: no fuel lever

For p-B11 (negligible fuel cost), the second channel disappears, and only the core-shrinkage channel remains. Because bremsstrahlung dominates the fusion energy split, even an efficient DEC captures only the 17% charged-particle margin and does not meaningfully reduce the required fusion power. Thermal LCOE is $42/MWh; VB DEC 60% is $42/MWh (core $1,538M vs $1,571M). The DEC overlay neither lowers the floor nor shrinks the core enough to pay for itself.
...
**7. D-He3 is the better DEC fuel, if you can get the He-3.** D-He3 has manageable bremsstrahlung (16-24% of fusion power) and a large charged fraction, making DEC a significant power path rather than a margin add-on. But purchased He-3 costs $29-44/MWh depending on architecture, far more than the entire BOP floor. Self-breeding eliminates this, but whether a D-He3 device can breed enough He-3 to sustain itself is undemonstrated.

**8. The p-B11 floor is industrial, not conversion-limited.** At baseline conditions, the p-B11 cost floor is $17/MWh, and a venetian blind DEC overlay raises it slightly rather than lowering it. With 83% of fusion energy radiated as bremsstrahlung, the thermal plant handles the majority of conversion and cannot be eliminated without an enormous increase in fusion power. At aggressive conditions, thermal and hybrid DEC converge to $7.1-7.3/MWh. The floor is dominated by buildings, electrical systems, O&M, and financing, so DEC's leverage, if any, has to come from elsewhere.
...
14. Smolentsev, S., "Modeling of Transport Processes in Liquid-Metal Fusion Blankets: Past, Present, and Future," *Fusion Science and Technology* (Oak Ridge National Laboratory) (2022). [Link](https://doi.org/10.1080/15361055.2022.2116905)
...
19. Hoffman, M.A., Campbell, R., & Logan, B.G., "Advanced fusion MHD power conversion using the CFAR cycle concept," *Fusion Technology* 15(2), 1254-1269 (1989). [OSTI 6276900](https://www.osti.gov/biblio/6276900)
20. Hoffman, M.A. et al., "Inertial fusion reactors using CFARII MHD conversion," *Fusion Engineering and Design* 22(1), 21-37 (1993). [ScienceDirect](https://www.sciencedirect.com/science/article/abs/pii/092037969390114W)