...
The workflow demonstrated in this post is manual: a human (often with an AI coding agent) reads a paper, extracts parameters, writes a 1costingfe script, and interprets the output. This works for two concepts. It does not scale to thirty.

We are building a more structured pipeline with four stages, of which the first and last are working today and the middle two are in progress.

**Stage one: concept ingestion.** This is working. For each of the 36 fusion concepts in our spanning set, an AI agent reads the published sources (papers, reports, patent filings), extracts parameters into a structured analysis document, and tags each value with its source citation and confidence level. The analysis goes through an automated feedback loop: a second agent reviews the extraction for missing parameters, unsupported claims, and internal inconsistencies, and the analysis is revised until it passes. The output is an 8-section document covering data availability, technical challenges, technology readiness, parameter tables, and modeling recommendations. We have completed this stage for 26 concepts.

**Stage two: system modeling.** This is in progress. The extracted parameters are encoded as a SysML v2 system model. SysML provides a formal structure: abstract types for costed components (with capital cost and CAS code), calculation definitions for LCOE and power balance, and concept-specific designs that specialize the abstractions with published values. Every numeric value carries a source citation in the model metadata.

The value of the SysML layer is that it separates what a plant *is* (structure, subsystems, interfaces) from what it *costs* (the CAS accounts). A tokamak and a stellarator share the same turbine plant, electrical plant, and heat rejection system; they differ in the reactor plant equipment. The SysML model makes this explicit: shared library definitions are reused across concepts, and concept-specific designs override only what differs. We have a working library covering the CAS hierarchy, foundation types, and IFE LCOE calculations, with two concept-specific designs (a heavy-ion fusion plant and a generic IFE plant).

**Stage three: code generation.** This is in progress. A code generator reads the SysML model and produces executable Python: a pipeline specification (a directed acyclic graph of calculations with their dependencies), wrapper modules with input/output validation, and placeholder implementations for the business logic. The generated code separates the wiring (auto-generated, not hand-edited) from the domain logic (hand-written or verified by a domain expert).

We have this working for two proof-of-concept pipelines: a minimal three-calculation chain test and a 16-module solar+battery LCOE model that runs end-to-end. The solar+battery example exercises the full loop: SysML model to generated pipeline to executed LCOE output. The fusion-specific code generation (connecting SysML fusion models to 1costingfe's CAS accounts) is the next step.

**Stage four: cost computation.** This is working, and is what this post demonstrates. 1costingfe computes LCOE from the CAS hierarchy with automatic differentiation for sensitivity analysis. Today the model setup scripts are written by hand (or by an AI agent); the goal is for Stage three to generate them automatically from the SysML model, so that adding a new concept means ingesting its papers and encoding its parameters, not writing Python.

The end state is a pipeline where a new fusion concept enters as a published paper and exits as a fully attributed LCOE estimate with sensitivity analysis, with human review at the parameter extraction and model validation steps but not at the code generation step. We are not there yet, but each stage is working independently, and the integration points are defined.
